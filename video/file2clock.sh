...
#
# The folder to put it in, or empty for the top level of the card.
#
# Same 8.3 rule as the name, minus the extension - the clock's
# filesystem has long filenames off, so a folder called "weekend films"
# cannot exist there either. Slashes separate levels, and each one is
# created on the clock if it is not already there.
#
if [ -n "$FOLDER" ]; then
    if ! printf '%s' "$FOLDER" \
        | grep -Eq '^[A-Za-z0-9_-]{1,8}(/[A-Za-z0-9_-]{1,8})*$'; then
        die "'$FOLDER' is not a folder name the clock can hold: up to 8 of
     A-Z a-z 0-9 _ - per level, '/' between levels, no extension"
    fi
fi

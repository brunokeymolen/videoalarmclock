...
/*
 * NN20Clock: this is Espressif's es8311 driver (registry version 1.0.0,
 * Apache-2.0, see LICENSE) with its I2C layer moved from the legacy
 * driver/i2c.h API to driver/i2c_master.h. See VENDORED.md for why it
 * is a copy rather than a dependency, and for the exact list of
 * changes. Everything else - the coefficient table and the register
 * sequences - is upstream's and should stay that way.
 */
#include <string.h>
#include "es8311.h"
...
/*
 * NN20Clock: the codec shares I2C port 0 with the touch controller, so
 * it runs at the bus speed the display set up. 100 kHz is what the
 * tryout used and what the datasheet calls standard mode.
 */
#define NN20CLOCK_ES8311_I2C_HZ 100000
...
void es8311_delete(es8311_handle_t dev)
{
    ...
    es8311_dev_t *es = (es8311_dev_t *) dev;
    if (es->dev != NULL) {
        (void) i2c_master_bus_rm_device(es->dev);
    }
    free(es);
}
...
es8311_handle_t es8311_create(i2c_master_bus_handle_t bus, const uint16_t dev_addr)
{
    ...
    const i2c_device_config_t dev_cfg = {
        ...
        .scl_speed_hz = NN20CLOCK_ES8311_I2C_HZ,
    };
    if (i2c_master_bus_add_device(bus, &dev_cfg, &sensor->dev) != ESP_OK) {
        ESP_LOGE(TAG, "ES8311 not on the I2C bus at 0x%02x", dev_addr);
        free(sensor);
        return NULL;
    }
